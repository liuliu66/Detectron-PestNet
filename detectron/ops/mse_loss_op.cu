...
namespace {
template <typename T>
__global__ void MSEKernel(
    const int n, const T* in, T* out, T beta) {
  // f(x) = x^2 / beta
  CUDA_1D_KERNEL_LOOP(index, n) {
    T val = in[index];
    T abs_val = abs(val);
    out[index] = val * val / beta;
  }
}
...
__global__ void MSEGradientKernel(
  ...
  CUDA_1D_KERNEL_LOOP(index, n) {
    ...
    out[index] = 2 * norm * d_loss * val / beta;
  }
}
} // namespace

template<>
bool MSELossOp<float, CUDAContext>::RunOnDevice() {
  auto& Y_hat     = Input(0);
  auto& Y         = Input(1);
  ...
  // Element-wise weighted MSE loss (can be used to specify a per-element
  // loss weight)
  // l := alpha_out * MSE(alpha_in * (y_hat - y))
  math::Mul<float, CUDAContext>(
...
template<>
bool MSELossGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& Y_hat      = Input(0);
  auto& Y          = Input(1);
  auto& alpha_in   = Input(2);
  auto& alpha_out  = Input(3);
  ...
  math::Mul<float, CUDAContext>(
      buff_.size(), buff_.data<float>(), alpha_in.data<float>(),
      buff_.mutable_data<float>(), &context_);
  // d_Y_hat := d_avg_loss / N * MSE'(alpha_in * (y_hat - y))
  MSEGradientKernel<float>
  <<<CAFFE_GET_BLOCKS(buff_.size()),
     CAFFE_CUDA_NUM_THREADS,
     0,
     context_.cuda_stream()>>>(
...
REGISTER_CUDA_OPERATOR(MSELoss,
                       MSELossOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(MSELossGradient,
                       MSELossGradientOp<float, CUDAContext>);
}  // namespace caffe2
