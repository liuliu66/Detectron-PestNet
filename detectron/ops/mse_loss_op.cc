...
REGISTER_CPU_OPERATOR(MSELoss, MSELossOp<float, CPUContext>);
REGISTER_CPU_OPERATOR(
    MSELossGradient,
    MSELossGradientOp<float, CPUContext>);

OPERATOR_SCHEMA(MSELoss)
    .NumInputs(4)
    .NumOutputs(1)
    .SetDoc(R"DOC(
MSE Loss is a minor variation of Huber loss in which the point of
transition between L2 loss and L1 loss is adjustable by a hyper-parameter beta:

  MSE(x) = x^2 / beta

MSE is used in Fast R-CNN and decendants as the loss function for bounding
box regression.

The loss computed by this op has a flexible form:

  scale / N * sum_i alpha_out[i] * SmoothL2(alpha_in[i] * (y_hat[i] - y[i])).
...
OPERATOR_SCHEMA(MSELossGradient)
    .NumInputs(5)
    .NumOutputs(1)
    .Input(
        0,
...
class GetMSELossGradient : public GradientMakerBase {
...
REGISTER_GRADIENT(MSELoss, GetMSELossGradient);
