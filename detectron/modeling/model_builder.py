...
import detectron.modeling.fr_fused_heads as fr_fused_heads
...
def rfcn(model):
    ...
    return build_generic_rfcn_model(model, 
        get_func(cfg.MODEL.CONV_BODY),
        add_roi_box_head_func=get_func(cfg.RFCN.ROI_BOX_HEAD),
        dim_reduce=1024,
        freeze_conv_body=cfg.TRAIN.FREEZE_CONV_BODY
        )
...
def _add_fast_rcnn_head(
    ...
    if model.train:
        if cfg.MODEL.WEIGHTED_LOSS:
            loss_gradients = fast_rcnn_heads.add_fast_rcnn_weighted_losses(model)
        else:
...
def _add_rfcn_head(
    model, add_roi_box_head_func, blob_in, dim_in, spatial_scale_in, dim_reduce
):
    """Add a Fast R-CNN head to the model."""
    blob_rfcn, dim_rfcn = add_roi_box_head_func(
        model, blob_in, dim_in, spatial_scale_in, dim_reduce
    )
    rfcn_heads.add_rfcn_outputs(model)
    if model.train:
        loss_gradients = rfcn_heads.add_rfcn_losses(model)
    else:
        loss_gradients = None
    return loss_gradients
...
def build_generic_rfcn_model(model, add_conv_body_func, add_roi_box_head_func=None, dim_reduce=None, freeze_conv_body=False):
