...
def add_ResNet_convX_body(model, block_counts, freeze_at=0):
    ...
    s2, dim_in = add_stage(model, 'res2', p, n1, dim_in, 256, dim_bottleneck, 1)
    if freeze_at == 2:
        model.StopGradient(s2, s2)
    s3, dim_in = add_stage(
        model, 'res3', s2, n2, dim_in, 512, dim_bottleneck * 2, 1
    )
    if freeze_at == 3:
        model.StopGradient(s3, s3)
    s4, dim_in = add_stage(
        model, 'res4', s3, n3, dim_in, 1024, dim_bottleneck * 4, 1
    )
    if freeze_at == 4:
        model.StopGradient(s4, s4)
    if len(block_counts) == 4:
        n4 = block_counts[3]
        s5, dim_in = add_stage(
            model, 'res5', s4, n4, dim_in, 2048, dim_bottleneck * 8,
        ...
        return s, 512, 1. / 4.
        #return s5, dim_in, 1. / 32. * cfg.RESNETS.RES5_DILATION
    else:
        return s4, dim_in, 1. / 16.
